...
class WeatherResponseMapperImpl @Inject constructor() : WeatherResponseMapper {
    ...
    override suspend fun map(response: WeatherResponse, measurementUnitsSet: MeasurementUnitsSet): List<Weather> =
        withContext(Dispatchers.Default) {


            return@withContext List(size = 7) { dailyIndex ->
                Weather(
                    date = dailyFormatter.format(LocalDate.parse(response.daily.date[dailyIndex], dailyResponseFormatter)),
                    ...
                    sunrise = hourlyFormatter.format(LocalDateTime.parse(response.daily.sunrise[dailyIndex], hourlyResponseFormatter)),
                    sunset = hourlyFormatter.format(LocalDateTime.parse(response.daily.sunset[dailyIndex], hourlyResponseFormatter)),
                    ...
                    hourlyWeather = List(size = 24) { hourlyIndex ->
                        val translatedIndex = dailyIndex * 24 + hourlyIndex
                        val hourlyLocalDateTime = LocalDateTime.parse(response.hourly.time[translatedIndex], hourlyResponseFormatter)
                        val isDay = hourlyLocalDateTime.hour in 4..22
                        val translatedWeather = TranslatedWeatherCode.fromWeatherCode(response.hourly.weatherCode[translatedIndex])
                        HourlyWeather(
